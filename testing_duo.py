...
if __name__ == "__main__":
    ...
    file = open(player1 + "_" + "duo" + "_" + player2 + "_" + player3, "w")
    ...
    while count < number_of_games:
        ...
        while p1:
            ...
            if first == player1_id:
                ...
                # get card
                rez = duo_first_action(vector, ma.player_hands[first], ma)
                ...
                # get state
                state1 = duo_second_fun(second, ma)

                # current vector of q table for this state
                if state1 in duo_second:
                    vector = duo_second[state1]
                else:
                    vector = [0 for i in range(duo_second_action_len)]
                    duo_second[state1] = vector

                # get card
                rez = duo_second_action(vector, ma.player_hands[second], ma)
                card, action_index1, num_possible_actions1 = qlearning.choose_action_testing(rez)
                ma.update_state(card, second)
                ...
                # get state
                state1 = duo_third_fun(third, ma)

                # current vector of q table for this state
                if state1 in duo_third:
                    vector = duo_third[state1]
                else:
                    vector = [0 for i in range(duo_third_action_len)]
                    duo_third[state1] = vector

                # get card
                rez = duo_third_action(vector, ma.player_hands[third], ma)
                card, action_index1, num_possible_actions1 = qlearning.choose_action_testing(rez)
                ma.update_state(card, third)
